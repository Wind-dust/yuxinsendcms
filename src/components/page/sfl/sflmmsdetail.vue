      ...
      <div class="box-card">
          ...
          <el-tabs class="eltabs" value="first" type="card">
            <el-tab-pane label="任务详细信息" name="first">
              <el-form class="form" label-width="250px">
                <el-form-item label="任务名称:"><span class="text">{{title}}</span></el-form-item>
                ...
                <el-form-item label="手机号:" :style="zan?'word-break:break-all':''"><span>{{task.mobile}}</span>
                </el-form-item>
        ...
        <div class="right">
          <div class="preview">
            <div class="inner">
              <div class="sms-text">
                <p style="color: #848a9f;font-weight: bold">{{title}}</p>
    ...
    methods: {
      ...
      getTask() {
        ...
        that.$request({
          ...
          data: {id: id},
          success(res) {
            that.title = res.mul.title
            that.template_id =res.mul.sfl_relation_id
            that.content = res.fram
            that.task = res.data
          }
        })
      }
